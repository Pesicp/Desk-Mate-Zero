...
class WeatherCard(BoxLayout):
    ...
    def _apply_forecast(self, result):
        ...
        for hh in hourly[start_index:start_index + 8]:
            ...
            b.add_widget(Image(source=ip, size_hint=(None, None), size=(75, 75), allow_stretch=True, keep_ratio=False, pos_hint={"center_x": 0.5, "center_y": 0.5}))
        ...
        for d in daily:
            ...
            b.add_widget(Image(source=ip, size_hint=(None, None), size=(110, 110), allow_stretch=True, keep_ratio=False, pos_hint={"center_x": 0.5, "center_y": 0.55}))
...
class RadioCard(RelativeLayout):
    ...
    def _on_country(self, country_code):
        ...
        def _worker():
            try:
                results = radio_api.get_stations_by_country(country_code, limit=100)
                KivyClock.schedule_once(
    ...
    def _show_stations(self, stations, title=""):
        ...
        for s in stations:
            row = BoxLayout(
                orientation="horizontal", size_hint_y=None, height=60, spacing=4
            )
...
class WeatherDisplayApp(App):
    def build(self):
        ...
        # Auto-switch to radio card after startup
        def _goto_radio(dt):
            for i, slide in enumerate(self.carousel.slides):
                if isinstance(slide, RadioCard):
                    ...
                    break
        KivyClock.schedule_once(_goto_radio, 1.5)

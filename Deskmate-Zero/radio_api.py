...
def search_stations(name: str, limit: int = 30):
    """Search stations across all fields (name, tags, etc.)."""
    if not name or not name.strip():
        return []
    base = _get_base_url()
    query = urllib.parse.quote(name.strip())
    url = f"{base}/json/stations/search?name={query}&limit={limit}&hidebroken=true"
    try:
        resp = requests.get(
...
def get_top_stations(country_code: str, limit: int = 30):
...
def get_stations_by_country(country_code: str, limit: int = 30):
